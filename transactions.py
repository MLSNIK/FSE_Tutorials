...
def add_transaction(
    ...
    if transaction_type not in TRANSACTION_TYPES:
        raise ValueError(f"Ivalid transaction type: {transaction_type}. Must be 'income' or 'expense'")
    
    if amount < 0:
        raise ValueError(f"Amount cannot be negative: {amount}. Please provide a positive vlaue.")
...
def calculate_balance(transactions: List[dict]) -> Decimal:
    ...
    Example:
        ...
        Decimal('4000')
    """
    income = get_income_total(transactions)
    expense = get_expense_total(transactions)
...
def check_budget(balance: Decimal, budget_limit: Decimal) -> Tuple[bool, str]:
    ...
    if balance <= budget_limit:
        return (True, f"Within budget. {format_currency(balance)} of {format_currency(budget_limit)}")
    else:
        overspend = balance - budget_limit
        return (False, f"Budget exceeded! Overspent by {format_currency(balance)} {format_currency(overspend)}.")
...
def display_transactions(transactions: List[dict]) -> None:
    ...
    Args:
        transactions: A list of transaction dictionaries.
    """
    income = get_income_total(list)
    expense = get_expense_total(list)
